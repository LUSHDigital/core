...
func TestTokeniser_ValidateToken_SuccessfulCase(t *testing.T) {
	testConsumer := &auth.Consumer{
		ID:        999,
		FirstName: "Testy",
		LastName:  "McTest",
	...
	_, err = tokeniser.ParseToken(authToken)
	if err != nil {
		t.Error(err)
	}
}

func TestTokeniser_ValidateToken_FailedCases(t *testing.T) {
	cases := []struct {
		name        string
		authToken   string
		expectedErr error
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, err := tokeniser.ParseToken(c.authToken)
			deepEqual(t, reflect.TypeOf(c.expectedErr), reflect.TypeOf(err))
		})
	}
}
